...
class User():
    ...
    - password: private string hash in MDS
    """
    ...
    def password(self, pwd):
        ...
        - Hash `pwd` is MD5 before assign to `__password`
        """
    ...
    def is_valid_password(self, pwd):
        ...
        - `False` if `__password` and the MD5 value of `pwd`
        """
        if pwd is None or type(pwd) is not str:
            return False
...
password")
    ...
    if user_1.is_valid_password("No pwd"):
